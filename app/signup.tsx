...
export default function Index() {
  ...
  useEffect(() => {
    const getCities = async () => {
      let response = await axios.get(`${config.apiBaseUrl}/cities`, { headers: { "Content-Type": "application/json", "Accept": "application/json" }, });
  ...
  const errorHandler = (error: any) => {
    ...
    if (error?.response?.data?.errors) {
      ...
      if (Array.isArray(errorsObject)) {
        errorsObject.forEach((err: any) => {
          message = `${err}`;
        });
      } else if (typeof errorsObject === 'object') {
        // If errors is an object (key: array of messages)
        Object.entries(errorsObject).forEach(([key, errArr]: any) => {
  ...
  const handleSendOtp = async () => {
    ...
    try {
      const response = await axios.post(`${config.apiBaseUrl}/otp/send`, { phone, user_verifications: false }, { headers: { "Content-Type": "application/json", "Accept": "application/json" }, });
